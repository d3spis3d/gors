...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        ...
        let next_id = dbstate.last_item_id + 1;

        dbstate.epics.insert(next_id, epic);
        dbstate.last_item_id = next_id;
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        let epic = dbstate
            .epics
            .get_mut(&epic_id)
            .ok_or(anyhow!("Epic not found: {}", epic_id))?;
        epic.stories.push(next_id);

        dbstate.stories.insert(next_id, story);
        dbstate.last_item_id = next_id;
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        let epic = dbstate
            ...
            .remove(&epic_id)
            .ok_or(anyhow!("Epic not found: {}", epic_id))?;
        let _stories: Vec<Option<Story>> = epic
            .stories
            .iter()
            .map(|s| dbstate.stories.remove(&s))
            .collect();
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        ...
        dbstate
            .stories
            .remove(&story_id)
            .ok_or(anyhow!("Story not found: {}", story_id))?;
...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        ...
        let dbstate: DBState = serde_json::from_str(&db_content)?;
